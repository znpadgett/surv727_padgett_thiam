...
dem <- search_tweets("#democrats OR #candidates OR #election2020 
                     OR #BIDEN OR #sanders OR #warren OR #harris 
...
```{r, eval=FALSE}
tweets$BS <-(str_detect(tweets$text, regex("#Sanders|@BernieSanders", 
                              ignore_case = TRUE)))
tweets$KH <-(str_detect(tweets$text, regex("#harris |@KamalaHarris",  
                              ignore_case = TRUE)))
tweets$JB <-(str_detect(tweets$text, regex("#biden |@JoeBiden", 
                              ignore_case = TRUE)))
tweets$EW <-(str_detect(tweets$text, regex("#warren |@ewarren", 
                              ignore_case = TRUE)))
tweets$PB <-(str_detect(tweets$text, regex("#buttigieg|@PeteButtigieg", 
                              ignore_case = TRUE)))
tweets$TS <-(str_detect(tweets$text, regex("#steyer | @TomSteyer", 
                              ignore_case = TRUE)))
tweets$AY <-(str_detect(tweets$text, regex("#yang| @AndrewYang", 
                              ignore_case = TRUE)))
tweets$BC <-(str_detect(tweets$text, regex("#booker | @BookerCory", 
                              ignore_case = TRUE)))
tweets$AK <-(str_detect(tweets$text, regex("#klobuchar | @amyklobuchar", 
                              ignore_case = TRUE)))
tweets$TG <-(str_detect(tweets$text, regex("#gabbard |@TulsiGabbard", 
                              ignore_case = TRUE)))
```
...
```{r, fig.cap="Pre- and Post-debate Google Trends Data, by Candidate"}
...
```{r, fig.cap="Pre- and Post-debate Polling Data, by Candidate"}
...
```{r, fig.cap="Comparison of Post-debate Twitter and Polling Data"}
...
```{r, fig.cap="Comparison of Post-debate Google Trends and Polling Data"}
graphing_data  %>%
  group_by(Pre_post, Candidate) %>%
  filter(Pre_post=="Post-debate") %>%
  ggplot() +
...
```{r, fig.cap="Comparison of Changes in Twitter and Polling Data"}
pre <-
graphing_data  %>%
...
```{r, fig.cap="Comparison of Changes in Google Trends and Polling Data"}
change_data %>%
  group_by(Pre_post, Candidate) %>%
  mutate(change_twitter = ((Post_twitter - Pre_twitter) / 
                             Pre_twitter) * 100, 
